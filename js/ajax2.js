...
function getData(combo)
{
	...
	if (combo == 'pais')
	{
		var curso = document.getElementById('curso').options[document.getElementById('curso').selectedIndex].value;
		var url = 'consultas_cotizador2.php?action=getPais&curso='+curso;
		var field = 'div_pais';
	...
	else if(combo=='ciudad')
	{
		...
		var url = 'consultas_cotizador2.php?action=getCiudad&curso='+ curso +'&id_pais='+id_pais;
	...
	{	
		...
		var url = 'consultas_cotizador2.php?action=getCentro&curso='+ curso +'&id_pais='+id_pais+'&id_ciudad='+id_ciudad;
	...
	else if(combo=='semanasCurso')
	{
		...
		var url = 'consultas_cotizador2.php?action=getsemanasCurso&centro='+ centro;
		document.getElementById(field).innerHTML = "<select class='select'><option>Cargando...</option></select>";
	}
		...
		var centro = document.getElementById('centro').options[document.getElementById('centro').selectedIndex].value;
		var curso = document.getElementById('curso').options[document.getElementById('curso').selectedIndex].value;
		var url = 'consultas_cotizador2.php?action=getleccionesSemana&centro='+ centro+'&curso='+curso;
		document.getElementById(field).innerHTML = "<select class='select'><option>Cargando...</option></select>";
	}
	else if (combo == 'jornadaLecciones'){
		var field = 'div_jornadaLecciones';
		var centro = document.getElementById('centro').options[document.getElementById('centro').selectedIndex].value;
		var url = 'consultas_cotizador2.php?action=getjornadaLecciones&centro='+ centro;
		...
		var centro = document.getElementById('centro').options[document.getElementById('centro').selectedIndex].value;
		var url = 'consultas_cotizador2.php?action=gettipoAlimentacion&tipo_alojamiento='+ tipo_alojamiento+'&centro='+centro;
...
function getDataCotizacion(){
	...
	var http = createRequestObject();
		...
		var alojamientoT=0;
		if (alojamiento == 'Si'){
			alojamientoT = document.getElementById('alojamientoT').options[document.getElementById('alojamientoT').selectedIndex].value;
		...
		var pasaje = document.getElementById('pasaje').options[document.getElementById('pasaje').selectedIndex].value;
		...
		//Modificado Disnet
		var complemento="";
		if(document.getElementById('PasajeMControl'))
		{
		var valorMPP=0;
			if(document.getElementById('PasajeMControl').value=="Si")
			{
				if(document.getElementById('PasajeMControl2').value=="Si")
				{
					valorMPP=document.getElementById('valor_pasaje').value;
				}
				else
				{
					valorMPP=document.getElementById('PasajeM').value;
				}
				complemento="&pasejeNV="+valorMPP+"&PasajeM="+document.getElementById('PasajeM').value+"&PasajeMControl="+document.getElementById('PasajeMControl').value;
			}
		}
		var complemento2="";
		if(document.getElementById('Descuento'))
		{
			...
			{
				var complemento2="&Descuento="+document.getElementById('Descuento').value;
			}
		}
		//Fin modificado disnet
		var url = 'consultas_cotizador2.php?action=cotizar&curso='+curso+'&pais='+pais+'&ciudad='+ciudad+'&centro='+centro+'&semanas_curso='+semanas_curso+'&lecciones_semana='+lecciones_semana+'&jornada_lecciones='+jornada_lecciones+'&alojamiento='+alojamiento+'&tipo_alojamiento='+tipo_alojamiento+'&semanas_alojamiento='+semanas_alojamiento+'&tipo_alimentacion='+tipo_alimentacion+'&traslado='+traslado+'&seguro='+seguro+'&pasaje='+pasaje+'&tipo_moneda='+tipo_moneda+'&alojamientoT='+alojamientoT+complemento+complemento2;//
